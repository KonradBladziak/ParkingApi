...
namespace DAL.Entity
{
    ...
    public class Miasto : IEntityTypeConfiguration<Miasto>
    {
        ...
        [MaxLength(50)]
        [RegularExpression(@"^[a-zA-Z'-'\s]{2,40}$", ErrorMessage = "Nazwa miasta może zawierać litery A-z oraz znaki specjalne typu '-'. " +
            "Ponadto musi mieć minimum 2 znaki długości a maximum 40 znaków długości!")]
        public string Nazwa { get; set; }

        [MaxLength(50)]
        [RegularExpression(@"^[a-zA-Z'-'\s]{2,20}$", ErrorMessage = "Nazwa województwa może zawierać litery A-z oraz znaki specjalne typu '-'. " +
